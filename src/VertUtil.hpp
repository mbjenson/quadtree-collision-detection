...
inline void computeBoxFrameVerts(
    ...
    // TODO: use sf::PrimativeType::Lines
...
inline void computeBoxFrameVertsFromVec(std::vector<Rect<float>>& rects, std::vector<sf::Vertex>& outVerts,
    ...
    for (const Rect<float> rect : rects) {
        ...
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top + lineWidth), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left, rect.top + lineWidth), color});
        // left
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left, rect.top), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + lineWidth, rect.top), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + lineWidth, rect.top + rect.height), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left, rect.top + rect.height), color});
        // right
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width - lineWidth, rect.top), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top + rect.height), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width - lineWidth, rect.top + rect.height), color});
        // bottom
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left, rect.top + rect.height - lineWidth), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top + rect.height - lineWidth), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left + rect.width, rect.top + rect.height), color});
        outVerts.emplace_back(sf::Vertex{sf::Vector2f(rect.left, rect.top + rect.height), color});
    }
}
...
inline void computePhysicsObjectVertsFromVec(
...
inline void toSfVertex(
    std::vector<Vec2f>& vecs, 
    std::vector<sf::Vertex>& outVerts, 
    sf::Color color
) {
